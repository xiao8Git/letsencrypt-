...
class PostArgParseSetupTest(test_util.TempDirTestCase):
    ...
    def setUp(self):
        ...
        self.config = mock.MagicMock(
            debug=False, logs_dir=self.tempdir, log_backups=1000, quiet=False,
...
class SetupLogFileHandlerTest(test_util.TempDirTestCase):
    ...
    def setUp(self):
        ...
        self.config = mock.MagicMock(logs_dir=self.tempdir, log_backups=42)
    ...
    def test_log_backups_used(self, mock_handler):
        self._call(self.config, 'test.log', '%(message)s')
        backup_count = mock_handler.call_args[1]['backupCount']
        self.assertEqual(self.config.log_backups, backup_count)
